...
export async function reassignLead(user: TokenPayload, leadId: number, assigneeId: number, reason: string) {
...
export async function bulkUploadLeads(user: TokenPayload, rawLeads: any[]) {
    ...
    if (rawLeads.length > MAX_BATCH_SIZE) {
      throw new AppError(400, `Cannot upload more than ${MAX_BATCH_SIZE} leads in a single request (received ${rawLeads.length}). Split into multiple batches.`);
    }
