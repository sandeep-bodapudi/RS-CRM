...
export async function createLead(
  ...
  if (existingLead) {
    ...
    if (!isDropped) {
      ...
      if (existingLead.assigned_to_id) {
        ...
        ).catch((err) => logger.error('[WebPush] Lead re-inquiry notify:', err));
      }
  ...
  const isChannelPartner =
    !isPublicSubmission && user.roles.includes(Roles.CHANNEL_PARTNER_MANAGER);
  ...
  return await p.$transaction(async (tx: import('@prisma/client').Prisma.TransactionClient) => {
    const lead = await tx.lead.create({
      data: {
        ...
        preferred_location:
          (dto.preferred_locations && dto.preferred_locations[0]) || dto.preferred_location || null,
    ...
    if (bestAssignee) {
      ...
      notifyEmployee(
        ...
        { skipDbNotification: true },
      ).catch((err) => logger.error('[WebPush] Lead auto-assign notify:', err));
    }
