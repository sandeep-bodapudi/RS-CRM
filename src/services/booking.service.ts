...
/**
 * Phase 2.8 (2026-09-06): the commercial Property lifecycle — LIVE → LOCKED
 * (createBooking) → BOOKED (confirmBooking), and released back to LIVE on
 * cancelBooking — deliberately lives here, outside `PropertyWorkflow`, per
 * the matching comment in `workflows/property.workflow.ts`: these
 * transitions are transactionally coupled to Booking/Customer/Opportunity/
 * Lead state in ways that engine's single-entity `canTransition()` can't
 * express. The inline guards in `createBooking` (LOCKED/BOOKED/SOLD/LIVE
 * checks below) are this domain's equivalent of a transition matrix.
 *
 * Found in passing, not fixed here (out of scope for this decision): nothing
 * in the codebase ever sets `Property.status = 'SOLD'` — the lifecycle stops
 * at BOOKED. `SOLD` is checked defensively in several places (this file,
 * `deriveAvailability`, `searchIntentBridge`) as if reachable, but has no
 * actual trigger (e.g. a "registration complete" action). Completing that is
 * a real feature addition needing its own product decision about when a
 * booking should be considered sold, not a workflow-engine question.
 */

interface CreateBookingInput {
  customer_id: number;
  /** Exactly one of property_id / project_unit_id. Validated by resolveInventoryRef. */
...
/** Full RRH booking form payload — mirrors the physical form fields */
interface BookingFormInput {
  ...
  sale_price_per_sqyd?: number;
  charges_per_sqyd?: number;
  // Financial summary
  emi_months?: number;
  emi_charges?: number;
  total_cost?: number;
  ...
  booking_amount_words?: string;
  // Referral
  referred_by?: string;
  referred_by_code?: string;
  // T&C acceptance
  tc_accepted_by_name?: string;
  // Legacy support
  is_legacy?: boolean;
  legacy_booking_date?: string; // ISO date string
  legacy_notes?: string;
}

/** Payload for the new booking initiation wizard (form + booking fields combined) */
interface InitiateBookingInput extends CreateBookingInput, BookingFormInput {
  // New customer creation (inline, optional)
  new_customer?: {
    first_name: string;
    last_name?: string;
    phone: string;
...
export class BookingService {
  ...
  private static async claimAndCreate(client: Prisma.TransactionClient, user: TokenPayload, dto: CreateBookingInput & { company_id: number }) {
    ...
    // attached to it.
    const ref = resolveInventoryRef(dto);
    const locked = await lockInventoryRow(client, ref);
    ...
    await claimInventoryLock(client, ref, booking.id, new Date(now.getTime() + LOCK_DURATION_MS));
  ...
  static async confirmBooking(user: TokenPayload, id: number) {
    ...
    const bookedRef = refFromRecord(booking);
    const bookedTitle = bookedRef ? await getInventoryTitle(p, bookedRef) : null;
    ...
    const inventoryDetail = bookedRef
      ? await getInventoryPortalDetail(p, bookedRef)
  ...
  static async cancelBooking(user: TokenPayload, id: number, reason: string = 'Booking cancelled') {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const cancelRef = refFromRecord(booking);
      if (cancelRef) {
        await releaseInventoryLock(tx, cancelRef, id);
      }
      return cancelled;
    });
   ...
   */
  static async initiateBooking(user: TokenPayload, dto: InitiateBookingInput) {
   ...
   */
  static async submitBookingForm(user: TokenPayload, id: number, formData: BookingFormInput) {
    const booking = await BookingService.getBookingById(user, id);

    if (!['DRAFT', 'MD_REJECTED'].includes(booking.form_status as string)) {
      throw new AppError(400, `Cannot resubmit a booking with form_status: ${booking.form_status}`);
    }
    ...
    await prisma.auditEvent.create({
      data: {
        ...
        entity_type: 'Booking',
        entity_id: id,
        old_value: booking.form_status as string,
        new_value: 'SUBMITTED',
        created_at: now,
      },
    });
   ...
   */
  static async getPendingMDApprovals(user: TokenPayload) {
   ...
   */
  static async mdApproveBooking(user: TokenPayload, id: number) {
    if (!user.roles.includes(Roles.MD)) {
      throw new AppError(403, 'Forbidden: Only the Managing Director can approve a booking form');
    }
   ...
   */
  static async mdRejectBooking(user: TokenPayload, id: number, reason: string) {
    if (!user.roles.includes(Roles.MD)) {
      throw new AppError(403, 'Forbidden: Only the Managing Director can reject a booking form');
    }

    const booking = await BookingService.getBookingById(user, id);

    if (booking.form_status !== 'SUBMITTED') {
      throw new AppError(400, `Cannot reject a booking that is not in SUBMITTED state`);
    }
