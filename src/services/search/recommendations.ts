import { rankByRequirementModel } from './engine';
import type { MatchCandidate, RequirementModel } from './types';
...
export function buildRecommendations(
  pool: MatchCandidate[],
  query: { location?: string; propertyType?: string; minBudget?: number; maxBudget?: number },
  excludeIds: number[],
  ...
  const ranked = rankByRequirementModel(candidates, loosened)
    ...
    .sort((a, b) => {
      ...
      if (aViewed !== bViewed) return bViewed - aViewed; // recently-viewed similar items surface first
