// Ported from the Sonthillu/Radha public-website BFFs' services/matching/engine.ts
// (identical in both forks, byte-for-byte apart from this file's import path)
// as part of merging AI-powered search into the CRM backend.
import type {
  Requirement,
  RequirementModel,
  RequirementPriority,
...
const PRIORITY_FACTOR: Record<RequirementPriority, number> = {
  LOW: 1,
  MEDIUM: 1.5,
  HIGH: 2,
};
...
function createEmptyExplanation(): MatchExplanation {
  return {
    score: 1,
    band: 'EXCELLENT',
    matched: [],
...
export function normalizeQueryToRequirements(query: {
  location?: string;
  propertyType?: string;
  listingType?: string;
  minBudget?: number;
...
function match(reason: string, requestedValue?: unknown, matchedValue?: unknown): FieldEvaluation {
...
function missing(reason: string, requestedValue?: unknown): FieldEvaluation {
  return { satisfaction: MISSING_SATISFACTION, status: 'MISSING', reason, requestedValue };
}

function unknown(reason: string, requestedValue?: unknown): FieldEvaluation {
...
function evaluatePropertyType(property: MatchCandidate, req: Requirement): FieldEvaluation {
...
function evaluateListingType(property: MatchCandidate, req: Requirement): FieldEvaluation {
...
function evaluatePossession(property: MatchCandidate, req: Requirement): FieldEvaluation {
...
function evaluateFacing(property: MatchCandidate, req: Requirement): FieldEvaluation {
...
function evaluateLocation(property: MatchCandidate, req: Requirement): FieldEvaluation {
...
function evaluatePrice(property: MatchCandidate, req: Requirement): FieldEvaluation {
  const range = req.value as [number, number];
  const min = range[0] ?? 0;
  ...
  if (max !== Infinity && pMin <= max && pMin >= min && pMax > max) {
    ...
    return partial(
      `Price range ${label} partially overlaps budget (max ${formatINR(max)})`,
  ...
  if (max !== Infinity) {
    ...
    if (pMin <= stretchLimit) {
      ...
      return partial(
        `Price ${label} is slightly above budget (max ${formatINR(max)})`,
        Math.round(satisfaction * 100) / 100,
        req.value,
        property.price,
      );
...
function evaluateParking(property: MatchCandidate, req: Requirement): FieldEvaluation {
...
function evaluateNumeric(property: MatchCandidate, req: Requirement): FieldEvaluation {
  ...
  const requested = req.value;
  const apply = (predicate: boolean, verb: string) =>
    predicate
  ...
  switch (req.operator) {
    ...
    case 'LESS_THAN':
      return apply(actual <= Number(requested), 'matches');
    case 'BETWEEN': {
      const [a, b] = requested as [number, number];
...
function evaluateAmenities(property: MatchCandidate, req: Requirement): FieldEvaluation {
...
export function evaluateField(property: MatchCandidate, req: Requirement): FieldEvaluation {
...
export function checkCandidateEligibility(
  ...
  for (const req of requirements) {
    ...
    if (req.importance !== 'HARD') continue;
...
function categorize(evalResult: FieldEvaluation, explanation: MatchExplanation): void {
...
export function rankByRequirementModel<T extends MatchCandidate>(
  properties: T[],
  requirementModel: RequirementModel,
): RankedProperty<T>[] {
...
function getMatchTier(band: MatchBand): RankedProperty<MatchCandidate>['tier'] {
  switch (band) {
    case 'EXCELLENT':
    case 'VERY_GOOD':
      return 'PRIMARY';
