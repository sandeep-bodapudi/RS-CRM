...
router.post(
  ...
  authenticateToken,
  requireAuthz(Permissions.PROPERTIES_VERIFY),
  validateRequestBody(PropertyResubmitSchema),
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const propertyId = parseInt(req.params.id, 10);
      if (isNaN(propertyId))
        return next({ name: 'AppError', statusCode: 400, message: 'Invalid ID format' });
      const updated = await PropertyService.resubmitProperty(req.user!, propertyId, req.body);
      ...
      logger.error('Property resubmit error:', error);
      if (error.status) {
        return next(error);
      }
