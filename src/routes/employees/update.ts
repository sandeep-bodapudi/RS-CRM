...
import { Roles, Permissions, EmployeeUpdateSchema } from '../../shared';
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!canViewSensitive) {
        ...
        if (body.pan_number !== undefined) updateData.pan_number = encryptData(body.pan_number);
        if (body.aadhaar_number !== undefined)
          updateData.aadhaar_number = encryptData(body.aadhaar_number);
        if (body.bank_name !== undefined) updateData.bank_name = encryptData(body.bank_name);
        if (body.bank_account_number !== undefined)
          updateData.bank_account_number = encryptData(body.bank_account_number);
        if (body.bank_ifsc !== undefined) updateData.bank_ifsc = encryptData(body.bank_ifsc);
        if (body.bank_branch !== undefined) updateData.bank_branch = encryptData(body.bank_branch);
      ...
      if (body.status !== undefined) {
        ...
        const msg =
          statusMessages[body.status] || `Your account status was changed to ${body.status}.`;
        notifyPromises.push(
          notifyEmployee(employeeId, {
            type: 'STATUS_CHANGED',
      ...
      const responseEmployee: any = { ...updatedEmp };
      if (canViewSensitive) {
        responseEmployee.pan_number = decryptData(updatedEmp.pan_number);
        responseEmployee.aadhaar_number = decryptData(updatedEmp.aadhaar_number);
        responseEmployee.bank_name = decryptData(updatedEmp.bank_name);
        responseEmployee.bank_account_number = decryptData(updatedEmp.bank_account_number);
        responseEmployee.bank_ifsc = decryptData(updatedEmp.bank_ifsc);
        responseEmployee.bank_branch = decryptData(updatedEmp.bank_branch);
      } else {
