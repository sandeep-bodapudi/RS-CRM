...
class AnalyticsService {
    ...
    static async getKpis(companyId, user) {
        ...
        const [pipelineMetrics] = await Promise.all([opportunity_service_1.OpportunityService.getPipelineMetrics(user)]);
        return {
            companyId,
    ...
    static async getSalesManagerDashboard(companyId, user) {
        ...
        const kpis = {
            ...
            conversionRate: allLeads.length > 0
                ? (allLeads.filter((l) => l.status === 'BOOKED').length / allLeads.length) * 100
                : 0,
        };
        ...
        const [headcount, newHiresThisMonth, onLeaveToday, onLeaveYesterday, leadsThisMonth, leadsLastMonth,] = await Promise.all([
            ...
            p.attendanceProposal.count({
                where: {
                    type: 'LEAVE',
                    status: 'APPROVED',
                    target_date: (0, time_1.getISTMidnightInstant)(todayStr),
            ...
            p.attendanceProposal.count({
                where: {
                    type: 'LEAVE',
                    status: 'APPROVED',
                    target_date: (0, time_1.getISTMidnightInstant)(yesterdayStr),
                    employee_id: { in: companyEmployeeIds },
                },
            }),
            p.lead.findMany({
