...
exports.bulkUploadLeads = exports.updateLeadStatus = exports.reassignLead = void 0;
...
async function reassignLead(user, leadId, assigneeId, reason) {
    ...
    return await p.$transaction(async (tx) => {
        ...
        const updated = await workflowEngine_1.WorkflowEngine.transitionLead(tx, leadId, newStatus, { actor: user, entity: lead }, {
    ...
    (0, notifyEmployee_1.notifyEmployee)(assigneeId, {
        type: 'TARGET_ASSIGNED',
        title: 'Lead Assigned to You',
...
async function updateLeadStatus(user, leadId, newStatus, notes, guardFields) {
    ...
    if (!(0, authorization_1.can)(user, shared_1.Permissions.LEADS_UPDATE, lead)) {
        throw new errors_1.AppError(403, 'Forbidden: You do not have permission to mutate this lead');
    }
    ...
    if (newStatus === 'DEMO_SCHEDULED') {
        ...
        if (!handlerId) {
            throw new errors_1.AppError(400, 'Please select who will handle this demo, or attach a project to the lead so the PM can be auto-assigned.');
        }
        ...
        if (!handler) {
            throw new errors_1.AppError(400, 'Selected demo handler was not found or is not active.');
        }
        ...
        if (!isEligible) {
            throw new errors_1.AppError(400, 'Selected employee is not eligible to handle a demo (must be a Project Manager, Agent, Sales Manager, Channel Partner Manager, or the Managing Director).');
        }
    }
    ...
    if (_guardFields.qualification) {
        ...
        if (q.budget_min !== undefined)
            entityContext.budget_min = q.budget_min;
        if (q.budget_max !== undefined)
            entityContext.budget_max = q.budget_max;
        if (q.property_type_preference !== undefined)
            entityContext.property_type_preference = q.property_type_preference;
        if (q.preferred_location !== undefined)
            entityContext.preferred_location = q.preferred_location;
    }
    ...
    const activityTypeForTransition = (from, to) => {
        if (to === 'DROPPED')
            return 'LEAD_DROPPED';
        if (to === 'RECOVERED_TO_POOL')
            return 'LEAD_RECOVERED';
        if (to === 'DEMO_SCHEDULED')
            return 'DEMO_SCHEDULED';
        if (to === 'DEMO_COMPLETED')
            return 'DEMO_COMPLETED';
        if (to === 'SITE_VISIT_SCHEDULED')
            return 'SITE_VISIT_REQUESTED';
        if (to === 'SITE_VISIT_COMPLETED')
            return 'SITE_VISIT_COMPLETED';
        if (to === 'NEGOTIATION')
            return 'STATUS_CHANGED'; // Opportunity auto-created (§4) below
        if (to === 'BOOKING_INITIATED')
            return 'STATUS_CHANGED'; // portal provision stub (§6) below
        if (to === 'BOOKED')
    ...
    if (isDrop && (lead.status === 'ASSIGNED' || lead.status === 'CONTACTED')) {
        ...
        if (exitReason === 'OTHER' && (lowerNotes.includes('unreachable') || lowerNotes.includes('not answering') || lowerNotes.includes('no response'))) {
            ...
            if (distinctDays.size < 5) {
                throw new errors_1.AppError(400, 'Cannot drop lead as unreachable without at least 5 days of CALL_LOGGED attempts.');
            }
        }
    }
    const finalLead = await p.$transaction(async (tx) => {
        const updateData = {
        ...
        if (newStatus === 'DEMO_SCHEDULED' && _guardFields.demo_scheduled_at && _guardFields.demo_handler_id) {
            ...
            (0, notifyEmployee_1.notifyEmployee)(demoHandlerId, {
                type: 'DEMO_SCHEDULED',
                title: `Demo Scheduled: ${lead.customer_name}`,
                message: `A demo for ${lead.customer_name} (${lead.lead_code}) has been scheduled for ${new Date(_guardFields.demo_scheduled_at).toLocaleString()}.`,
        ...
        if (newStatus === 'QUALIFIED' && guardFields?.qualification) {
            ...
            if (q.budget_min !== undefined)
                updateData.budget_min = q.budget_min;
            if (q.budget_max !== undefined)
                updateData.budget_max = q.budget_max;
            if (q.property_type_preference !== undefined)
        ...
        if (newStatus === 'NEGOTIATION') {
            ...
            if (interestedLeadId && interestedLeadId === leadId) {
                await opportunity_service_1.OpportunityService.createFromLeadTx(tx, lead, user.employeeId || 1, interested.property_id);
            }
            else {
                await opportunity_service_1.OpportunityService.createFromLeadTx(tx, lead, user.employeeId || 1);
            }
        }
        // §6: customer-portal provisioning stub on BOOKING_INITIATED
        if (newStatus === 'BOOKING_INITIATED') {
            await customerPortal_service_1.CustomerPortalService.provisionStub(tx, lead, user);
        }
    ...
    if (!isRecover && finalLead.assigned_to_id) {
        (0, notifyEmployee_1.notifyEmployee)(finalLead.assigned_to_id, {
...
async function bulkUploadLeads(user, rawLeads) {
    ...
    if (rawLeads.length > MAX_BATCH_SIZE) {
        throw new errors_1.AppError(400, `Cannot upload more than ${MAX_BATCH_SIZE} leads in a single request (received ${rawLeads.length}). Split into multiple batches.`);
    }
    const results = {
    ...
    for (const e of existingLeads) {
        if (e.phone)
            existingPhonesMap.set(e.phone, e);
        if (e.email)
            existingEmailsMap.set(e.email, e);
    }
        ...
        for (const item of chunk) {
            ...
            try {
                ...
                await p.$transaction(async (tx) => {
                    const leadCode = await (0, shared_2.generateNextLeadCode)(); // Inside transaction to ensure unique code sequentially
                    const bestAssignee = await (0, distributionService_1.findBestAssigneeForLead)(user.companyId);
...
exports.bulkUploadLeads = bulkUploadLeads;
