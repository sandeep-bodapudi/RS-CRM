...
async function createLead(user, dto, opts) {
    ...
    if (!dto.phone) {
        throw new errors_1.AppError(400, 'Phone number is required for lead creation.');
    }
    ...
    const leadCode = await (0, shared_2.generateNextLeadCode)();
    const isChannelPartner = !isPublicSubmission && user.roles.includes(shared_1.Roles.CHANNEL_PARTNER_MANAGER);
    ...
    if (isChannelPartner) {
        ...
        if (!dto.external_agent_name || !dto.external_agent_phone || !dto.external_agent_associate_id) {
            throw new errors_1.AppError(400, 'External agent name, phone, and associate ID are required for Channel Partner leads.');
        }
    }
    ...
    const leadScore = (0, shared_2.calculateLeadScore)(dto);
    ...
    return await p.$transaction(async (tx) => {
        ...
        if (dto.preferred_locations && dto.preferred_locations.length > 0) {
            await (0, shared_2.syncLeadPreferredLocations)(tx, lead.id, dto.preferred_locations);
        }
