...
exports.reassignDemo = exports.cancelDemo = exports.completeDemo = exports.declineDemo = exports.acceptDemo = exports.getDemo = exports.listDemos = void 0;
const prisma_1 = require("../lib/prisma");
const notifyEmployee_1 = require("../utils/notifyEmployee");
const logger_1 = require("../utils/logger");
const p = prisma_1.prisma;
async function listDemos(user, filters) {
    ...
    for (const demo of demos) {
        if (demo.lead && demo.handler_id !== user.employeeId) {
            delete demo.lead.phone;
            delete demo.lead.email;
        }
    }
    return demos;
}
exports.listDemos = listDemos;
async function getDemo(user, demoId) {
...
async function acceptDemo(user, demoId, notes) {
    const demo = await p.demo.findFirst({
        where: { id: demoId, lead: { company_id: user.companyId } },
    ...
    if (!demo)
        throw { status: 404, message: 'Demo not found' };
    if (demo.accepted_at)
        throw { status: 409, message: 'Demo already accepted' };
    ...
    // Web push to handler
    (0, notifyEmployee_1.notifyEmployee)(demo.handler_id, {
        type: 'DEMO_ACCEPTED',
    ...
    }, { skipDbNotification: true }).catch((err) => logger_1.logger.error('[WebPush] Demo accept:', err));
...
exports.acceptDemo = acceptDemo;
async function declineDemo(user, demoId, notes) {
    const demo = await p.demo.findFirst({
        where: { id: demoId, lead: { company_id: user.companyId } },
        include: { lead: true, handler: true },
    });
...
exports.declineDemo = declineDemo;
async function completeDemo(user, demoId, notes) {
    const demo = await p.demo.findFirst({
        where: { id: demoId, lead: { company_id: user.companyId } },
        include: { lead: true, handler: true },
    });
...
async function cancelDemo(user, demoId, notes) {
    const demo = await p.demo.findFirst({
        where: { id: demoId, lead: { company_id: user.companyId } },
        include: { lead: true, handler: true },
    });
...
async function reassignDemo(user, demoId, newHandlerId, reason) {
    const demo = await p.demo.findFirst({
        where: { id: demoId, lead: { company_id: user.companyId } },
        include: { lead: true, handler: true },
    });
    ...
    if (demo.status !== 'PENDING')
        throw { status: 409, message: 'Can only reassign PENDING demos' };
    const newHandler = await p.employee.findFirst({
        where: { id: newHandlerId, company_id: user.companyId },
    });
    if (!newHandler)
        throw { status: 404, message: 'Target handler not found' };
    ...
    if (demo.handler_id && demo.handler_id !== newHandlerId) {
        ...
        }, { skipDbNotification: true }).catch((err) => logger_1.logger.error('[WebPush] Demo reassign old:', err));
    }
    // Web push to new handler
    (0, notifyEmployee_1.notifyEmployee)(newHandlerId, {
        type: 'DEMO_ASSIGNED',
        title: `Demo Assigned to You: ${demo.lead.customer_name}`,
        message: `A demo for ${demo.lead.customer_name} (${demo.lead.lead_code}) has been routed to you.`,
    }, { skipDbNotification: true }).catch((err) => logger_1.logger.error('[WebPush] Demo reassign new:', err));
    return await p.demo.findFirst({
        where: { id: demoId },
        include: { lead: true, handler: true },
    });
}
exports.reassignDemo = reassignDemo;
