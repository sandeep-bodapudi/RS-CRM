...
exports.PricingService = exports.getEffectiveRulesForProperty = exports.ruleToEngineRuleProperty = exports.getEffectiveRules = exports.amenityToEngineRule = exports.ruleToEngineRule = void 0;
...
exports.ruleToEngineRule = ruleToEngineRule;
/**
 ...
 */
function amenityToEngineRule(pa) {
    return {
        id: -pa.amenity_id,
...
exports.amenityToEngineRule = amenityToEngineRule;
/**
 ...
 */
function ruleToEngineRuleProperty(r) {
    return {
        id: r.id,
        label: r.label,
        kind: r.kind,
...
exports.ruleToEngineRuleProperty = ruleToEngineRuleProperty;
...
async function persistComputation(tx, kind, id, computation, overridePrice) {
    ...
    await tx.priceLine.deleteMany({ where: { [idField]: id } });
    ...
    const finalPrice = (0, engine_1.resolveFinalPrice)(computation.calculated_price, overridePrice);
    const summary = {
...
class PricingService {
     ...
     */
    static async recalculateUnit(unitId) {
    ...
    static async recalculateProperty(propertyId) {
     ...
     */
    static async previewForProject(user, projectId, input) {
        ...
        let areaSqft = null;
        let plotAreaSqyd = input.plot_area_sqyd ?? null;
        if (input.area_value != null && input.area_unit) {
            const normalized = (0, measurement_1.normalizeArea)(input.area_value, input.area_unit);
            areaSqft = normalized.area_sqft;
            if (plotAreaSqyd == null)
                plotAreaSqyd = normalized.area_sqyd;
        }
        const engineInput = {
        ...
        return (0, engine_1.computePrice)(engineInput, engineRules);
    }
    /**
     ...
     */
    static async previewForProperty(user, propertyId, input) {
        ...
        if (!property)
            throw { status: 404, message: 'Property not found' };
        const engineRules = await getEffectiveRulesForProperty(propertyId);
        let areaSqft = input.area_value != null && input.area_unit == null ? input.area_value : null;
        let plotAreaSqyd = input.plot_area_sqyd ?? null;
        if (input.area_value != null && input.area_unit) {
            const normalized = (0, measurement_1.normalizeArea)(input.area_value, input.area_unit);
            areaSqft = normalized.area_sqft;
     ...
     */
    static async previewRecalculateProject(user, projectId) {
        const scope = await (0, dataScope_1.buildProjectScope)(user);
        const project = await p.project.findFirst({ where: { id: projectId, ...scope } });
        if (!project)
            throw { status: 404, message: 'Project not found' };
    ...
    static async applyRecalculateProject(user, projectId) {
        const scope = await (0, dataScope_1.buildProjectScope)(user);
        const project = await p.project.findFirst({ where: { id: projectId, ...scope } });
        if (!project)
            throw { status: 404, message: 'Project not found' };
