...
class LeadService {
     ...
     */
    static async generateNextLeadCode() {
    ...
    static async getDistributionMonitor(companyId) {
    ...
    static async createLead(user, dto) {
        // ─────────────────────────────────────────────────────────────────────────
    ...
    static async bulkUploadLeads(user, rawLeads) {
        const results = {
    ...
    static async reassignLead(user, leadId, assigneeId, reason) {
        ...
        return await p.$transaction(async (tx) => {
            ...
            const updated = await workflowEngine_1.WorkflowEngine.transition(tx, leadId, newStatus, { actor: user, entity: lead }, {
    ...
    static async updateLeadStatus(user, leadId, newStatus, notes, guardFields) {
        ...
        return await p.$transaction(async (tx) => {
            ...
            const updated = await workflowEngine_1.WorkflowEngine.transition(tx, leadId, newStatus, { actor: user, entity: entityContext }, updateData);
            ...
            if (newStatus === 'RECOVERED_TO_POOL') {
                const bestAssignee = await (0, distributionService_1.findBestAssigneeForLead)(user.companyId);
                if (bestAssignee) {
                    finalUpdated = await workflowEngine_1.WorkflowEngine.transition(tx, leadId, 'ASSIGNED', { actor: user, entity: { ...entityContext, status: 'RECOVERED_TO_POOL' } }, {
     ...
     */
    static async distributeUnassignedPoolLeads(companyId) {
    ...
    static async triggerLeadRecoveryForProperty(propertyId) {
