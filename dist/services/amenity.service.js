...
function hasCatalogAccess(user) {
    return (user.permissions || []).includes(shared_1.Permissions.PROJECTS_UPDATE);
}
/**
...
class AmenityService {
    ...
    static async updateCatalogAmenity(user, amenityId, data) {
        if (!hasCatalogAccess(user)) {
            throw { status: 403, message: 'Forbidden: Missing projects.update permission' };
        }
    ...
    static async deleteCatalogAmenity(user, amenityId) {
        if (!hasCatalogAccess(user)) {
            throw { status: 403, message: 'Forbidden: Missing projects.update permission' };
        }
        const amenity = await p.amenity.findFirst({ where: { id: amenityId, company_id: user.companyId } });
    ...
    static async listProjectAmenities(user, projectId) {
     ...
     */
    static async setProjectAmenity(user, projectId, amenityId, data) {
     ...
     */
    static async applySelectedUnits(projectId, amenityId, unitIds) {
