...
exports.calculateAttendancePoints = exports.PERFORMANCE_TIER_CUTOVER = exports.calculateAttendanceStatus = exports.getISTMonthRange = exports.getISTMidnightInstant = exports.toHolidayDateKey = exports.getISTDayOfWeek = exports.getISTComponents = void 0;
...
exports.getISTComponents = getISTComponents;
/**
 ...
 */
const getISTDayOfWeek = (dateString) => {
 ...
 */
const toHolidayDateKey = (dateString) => new Date(`${dateString}T00:00:00Z`);
exports.toHolidayDateKey = toHolidayDateKey;
/**
 ...
 */
const getISTMidnightInstant = (dateString) => new Date(`${dateString}T00:00:00+05:30`);
exports.getISTMidnightInstant = getISTMidnightInstant;
/**
 ...
 */
const getISTMonthRange = (year, month) => {
 ...
 */
const calculateAttendanceStatus = (checkInDate = new Date(), hasApprovedProposal = false, employmentType = 'FULL_TIME') => {
    // PART_TIME / CONTRACT / INTERN — no penalty marks, duration tracked via check-in/out
    if (employmentType === 'PART_TIME' ||
        employmentType === 'CONTRACT' ||
        employmentType === 'INTERN') {
        return shared_1.AttendanceStatus.PRESENT;
    }
    // FULL_TIME — existing cutoff-based marks
    const { hours, minutes } = (0, exports.getISTComponents)(checkInDate);
    ...
    if (totalMinutes <= cutoff1130) {
        return hasApprovedProposal ? shared_1.AttendanceStatus.APPROVED_LATE : shared_1.AttendanceStatus.LATE;
    }
    // After 11:30 AM IST -> Half day rule
    return hasApprovedProposal ? shared_1.AttendanceStatus.APPROVED_HALF_DAY : shared_1.AttendanceStatus.HALF_DAY;
};
 ...
 */
exports.PERFORMANCE_TIER_CUTOVER = new Date('2026-09-10T00:00:00+05:30');
/**
 ...
 */
const calculateAttendancePoints = (status, checkInDate, employmentType = 'FULL_TIME') => {
    if (status === shared_1.AttendanceStatus.APPROVED_LATE || status === shared_1.AttendanceStatus.APPROVED_HALF_DAY) {
        return 0.0;
    }
    if (status === shared_1.AttendanceStatus.LATE || status === shared_1.AttendanceStatus.HALF_DAY) {
        return -1.0;
    }
    if (status !== shared_1.AttendanceStatus.PRESENT) {
    ...
    if (employmentType !== 'FULL_TIME' || !checkInDate || checkInDate < exports.PERFORMANCE_TIER_CUTOVER) {
        return 0.5;
    }
    const { hours, minutes } = (0, exports.getISTComponents)(checkInDate);
    const totalMinutes = hours * 60 + minutes;
    ...
    if (totalMinutes < cutoff1000)
        return 1.0;
    if (totalMinutes < cutoff1015)
        return 0.5;
